...
function execute2(ops, router, changed) {
	...
	//default   act as middleware before they get to the projects. Why?
	...
	ops.command.split('+').forEach(function(command) {
		...
		steps.push(function() {
			...
			if(changed) router.push('user/notification', { message: command + ' ' + cops.args[0], title: 'Cupboard' });


			router.pull('command/execute/' + command, cops, function(err, response) {
...
var execute = exports.execute = function(ops) {
	...
	bootstrap.init(function(router) {
		...
		if(!ops.command) ops.command = 'help';
...
var getProjects = exports.getProjects = function(projects, callback) {
	...
	bootstrap.init(function(router) {
		...
		router.pull('projects', { target: projects.join('+') }, function(err, response) {
