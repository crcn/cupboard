...
function execute2(ops, router) {
	...
	ops.command.split('+').forEach(function(command) {
		...
		steps.push(function()
		{
			...
			router.pull('command/execute/' + command, Structr.copy(ops), function(response) {
...
var execute = exports.execute = function(ops) {
	...
	bootstrap.init(function(router) {
		...
		if(ops.watch && target) {
			...
			console.log('Watching target projects for any changes'.grey);
