	...
	router.on({
		...
		'pull command/execute/init OR commmand/execute/add': function(req, res) {
			...
			Project.findOne({ _id: projectId }, function(err, item) {
				...
				//n 0.4 vs 0.6
				relPath = path.relative ? path.relative(lnPath, projectPath) : projectPath;
		...
		'pull projects OR projects/:target': function(req) {
			...
			Project.find(ops.projectName, function(err, projects) {
				...
				if(!req.next()) vine.result(projects).end(res);
			});
		},
