...
exports.plugin = function(router) {
	...
	function exec2(cmd, cwd, callback) {
		...
		var proc = exec(cmd, { cwd:  cwd || process.env.CONF_DIR }, function(err, stdout, stderr) {
	...
	router.on({
		...
		'pull command/execute/install': function(req, res) {
			...
			console.log('installing plugins: %s', req.query.args.join(', ').bold);
			
			// console.log(req.query.args.join(' '))
